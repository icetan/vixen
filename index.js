        ...
        if (n.nodeType === node.TEXT_NODE && (str = n.nodeValue)) {
          ...
          if (m) {
            chains = m.map(function(chain) {
              return chain.slice(2, -2).split(pipe);
            });
            // Add event listeners
            ...
            chains.forEach(function(chain) {
              ...
              bucket(binds, chain[0].trim().split('.')[0], nodeId);
            });
          }
        }
      });
          ...
          for (i in list) if (list.hasOwnProperty(i))
            (function(value, i) {
              var clone = template.cloneNode(true),
                  maps, nodeId;
              maps = traverse(clone);
              orig_[alias[0].trim()] = value;
              if (alias[1]) orig_[alias[1].trim()] = i;
